...
import java.util.LinkedList;
	...
	// 3) 객체만 저장 가능
	// 	  (기본 자료형은 안됨 -> Wrapper클래스를 이용하면 된다.)
...
public class ListService {
	...
	public void method1() {
		...
		// E(Element) : 요소를 뜻하는 상징적인 글자 (자료형 X)
						// == Objext, String, sutdent 등
		// 					  자료형 가리지 않고 들어갈 수 있다.
		
		// 1. boolean add(E e) : 마지막 요소로 추가
		list.add("아무거나");
		list.add(new Object());
		list.add(123123); // -> int가 Integer(Wrapper Class)로 변환 -> 컴파일러가 자동으로 변환
		
		list.add(3.123123); // 크기 3초과 했을때 오류 발생 확인
		list.add(false); 	// -> 오류 X
							// 크기가 자동으로 늘어나기 때문
		System.out.println(list);
		
		
		// 2. E get(int index) : List에서 index 번째 요소 반환받기
	...
	public void method2() {
		...
		strList.add("불닭볶음면");
		strList.add("카레우동");
		//strList.add(123); //String이 아니라서 error
		...
		//5. E Set(int index, E e) : index번째 요소를 e로 수정 -> 수정 전 요소가 반환됨
	...
	public void bookManage() {
		...
		do {
			...
			switch(input) {
				...
				case 3 :
					System.out.println("조회할 책의 index 입력 : ");
