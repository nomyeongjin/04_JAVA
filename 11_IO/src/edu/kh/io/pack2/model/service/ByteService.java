...
public class ByteService {
	...
	// IO 관련 코드는 IOExeption이 발생될 가능성이 높다!
	// -> 거의 모든 메서드가 throws IOException이 쓰여있음
	...
	public void fileByteOutput() {
		...
		try {
			...
			*/
			...
			// System.nanoTime() : 1970.01.01. 09:00:00 기준으로
			//					   현재 시간까지 차이를 ns(nano second단위)로 반환
			// 1ms(밀리) == 1/1000초
			// 1us (마이크로) == 1/1000ms
	...
	public void bufferedFileByteOutput() {
		...
		/* BufferedOuputStream / BufferedInputStream
		 * 
		...
		FileOutputStream fos = null; // try, finally 모두에서 사용 예정
		...
		try {
			...
			try {
				...
				// 보조 스트림 close() 시
				// 보조 스트림 생성에 사용된 기반 스트림도 같이 close됨()
	...
	public void BufferedFileByteInput() {
		...
		try {
			...
			try { // 보조 스트림만 close해도 기반 스트림도 같이 close됨
	 ...
	 * 복사완료 : /io_test/20240215/노래가사_copy.txt
	 * 
	 * 1) 입력된 경로에 파일이 있는지 검사 : File.exists();
	 * 2) 있으면 파일 내용을 모두 읽어오기 : FileInputStream + BufferedInputStream
	 * 3) 읽어온 내용을 같은 위치에 "파일명_copy" 이름으로 출력 : FileOuputStream + BufferedOuputStream
	 * 
	...
	public void fileCopy() {
		...
		try {
			...
			// sb : /io_test/20240215/노래가사_copy.txt
