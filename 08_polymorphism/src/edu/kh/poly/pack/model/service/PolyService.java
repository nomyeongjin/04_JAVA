package edu.kh.poly.pack.model.service;

import edu.kh.poly.pack.model.dto.Galaxy;
import edu.kh.poly.pack.model.dto.Iphone;
import edu.kh.poly.pack.model.dto.SmartPhone;
...
public class PolyService {
	 ...
	 *    -> 조건 : 메서드명 동일, 매개 변수 개수, 타입, 순서 하나라도 다름
	 *    
	...
	public void method1() {
		...
		// SmartPhone = Iphone / Galaxt
		// ->원래는 컵퓨터 값 처리 원칙 위배 (자동 형변환 대상 x)
		// -> 자식 객체 내부에 존재하는 부모타입 객체(SmartPhone)만 참조하게 됨
		
		// -> 자식 색체의 모습이 부모 객체로 변한 것처럼 보인다.
		//    (자신 객체가 부모  객체의 모습도 가지고 있음 == 다형성)
		
		SmartPhone s1 = new Iphone();
		SmartPhone s2 = new Galaxy();
		
		// 상속 받은 부모 기능 호출 -> 가능 O
		s1.setDisplay("레티나 디스 플레이");
	...
	/**
	 * 다형성 확인
	 * (자식 타입) 부모 타입 참조 변수 -> Down Castiong, 다운 캐스틴
	 * 
	 * - 업 캐스팅 된 객체 (자식 ->부모)를 참조하던 부모타입 참조 변수를
	 *   자식 타입으로 강제 형변환하여 다시 자식 객체를 참조하도록 변경(부모 -> 자식)
	 */
	public void method2() {
		...
		// 다운 캐스팅 + 얕은 복사 (주소만 복사)
		Iphone i1 = ((Iphone)s1);// s1을 Iphone으로 강제 형변환 -> 주소를 i1 대입
	...
	public void method3() {
		...
		// void edu.kh.poly.pack.model.dto.SmartPhone.setAp(String ap)
		s1.setAp("M1");
	...
	public void method4() {
		...
		// 업캐스팅
		SmartPhone s1 = new Galaxy("AMOLED","SKT","스냅드래곤",17);
		
		// s1.toString() 작성시
		// SmartPhone의 toString() 호출된다고 표시굄 == 정적 바인딩
	...
	public void method5() {
		...
		// 부모 참조 변수 = 자식 객체 (업캐스팅)
		arr[2]= new Galaxy("AMLED", "LG", "Snap Drangon3", 14);
		...
		for(SmartPhone s : arr) {// [1], [2] 인덱스에서 업캐스팅 적용
			...
			// 매 반복시마다 arr의 요소를 하나씩 꺼내 s에 저장
			
			System.out.println(s.toString());
			
			// [0] : SmartPhone.toString() 호출
			// [1] : Ipone.toString() 호출 (동적 바인딩)
	...
	public void method6() {
		...
		Galaxy galaxy= new Galaxy("AMLED", "LG", "Snap Drangon3", 14);
	...
	public void printToString(SmartPhone s) {
		System.out.println("[전달받은 객체의 toString() 출력]");
...
//	}
	...
	public void method7() {
		...
		printToString(createPhone(3)); // new Galaxy()로 생성된 객체 주소
	...
	public void method8() {
		...
		System.out.println("s1 instanceof String : error");
		//System.out.println(s1 instanceof String);
		//Incompatible conditional operand types SmartPhone and String
		// -> 상속관계가 아니라서 instanceof 검사 자체를 진행 못함
